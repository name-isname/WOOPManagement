<template>
  <div class="card" @contextmenu.prevent="$emit('context', { originalEvent: $event, item })" @mouseenter="hover=true" @mouseleave="hover=false">
    ...
    <div class="actions" v-show="hover">
      ...
      <button class="icon" title="链接 AI" @click.stop="$emit('link-ai', item)">🤖</button>
    </div>
...
.actions{ position:absolute; top:8px; right:8px; display:flex; gap:6px; z-index:20 }
.icon{ background: color-mix(in oklab, var(--surface) 60%, #0000000f); border:1px solid var(--surface-border); border-radius:8px; padding:4px 6px; cursor:pointer; min-width:28px; height:28px; display:flex; align-items:center; justify-content:center }
.icon:hover{ background: color-mix(in oklab, var(--surface) 40%, #0000001a) }
.wish{ margin-top:6px; color:#64748b }
</style>
