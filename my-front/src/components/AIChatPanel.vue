<template>
  <aside class="ai-panel border-l p-4" :style="{ width: panelWidth + 'px' }" ref="panelEl">
    ...
    <div v-if="project" class="mb-3 text-sm text-slate-600">当前项目： <strong>{{ project.name || project.title }}</strong>
    </div>

    <div class="chat-window mb-3" ref="chatEl">
      <div v-for="(m, i) in messages" :key="i" class="msg" :class="m.role">
        <div class="avatar" aria-hidden="true">{{ m.role === 'user' ? '🧑' : (m.role === 'system' ? '⚙️' : '🤖') }}
        </div>
        <div class="bubble" :class="m.role === 'assistant' ? 'ai' : m.role">
          <template v-for="(seg, si) in parseSegments(m.content)" :key="si">
            <pre v-if="seg.type === 'code'" class="code"><code>{{ seg.content }}</code></pre>
            <div v-else class="text" v-html="renderMarkdown(seg.content)"></div>
          </template>
          <button class="copy" title="复制" @click="copy(m.content)">复制</button>
        </div>
      </div>
    </div>
    ...
    <div class="composer">
      ...
      <div class="actions">
        <button class="btn ghost" @click="clearChat" :disabled="!messages.length">清空上下文</button>
        <span class="hint" v-if="statusMsg">{{ statusMsg }}</span>
        <button class="btn primary" @click="send" :disabled="sending || !input.trim()">{{ sending ? '发送中…' : '发送'
        }}</button>
      </div>
    </div>
  </aside>
...
async function send() {
  ...
  try {
    ...
    // 将当前完整对话上下文（由前端维护）发送给后端
    const contextToSend = messages.value.map(m => ({ role: m.role, content: m.content }))

    // 发送请求
    const res = await fetch(`${apiBase}/ai/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: contextToSend }) })
    if (!res.ok) {
      const txt = await res.text()
      throw new Error(txt || ('Status ' + res.status))
    }

    // 在界面上添加一个占位的 AI 消息，用于拼接流式回复
    ...
    while (true) {
      ...
      for (const part of parts) {
        ...
        for (const l of lines) {
          if (l.startsWith('event:')) eventType = l.replace(/^event:\s*/, '').trim()
          if (l.startsWith('data:')) dataLines.push(l.replace(/^data:\s*/, ''))
        }
        const dataStr = dataLines.join('\n')

        if (!eventType || eventType === 'message' || eventType === 'data') {
          // 这是正常的数据事件，尝试解析 JSON，否则直接作为文本追加
          try {
            const obj = JSON.parse(dataStr)
            const chunk = (obj && (obj.text ?? obj.chunk ?? obj.data)) || dataStr
            const aiMsg = messages.value[messages.value.length - 1]
            if (aiMsg && aiMsg.role === 'assistant') aiMsg.content = (aiMsg.content || '') + chunk
            else messages.value.push({ role: 'assistant', content: chunk })
          } catch (e) {
            const aiMsg = messages.value[messages.value.length - 1]
            if (aiMsg && aiMsg.role === 'assistant') aiMsg.content = (aiMsg.content || '') + dataStr
            else messages.value.push({ role: 'assistant', content: dataStr })
          }
          await nextTick(); scrollToBottom()
        } else if (eventType === 'done') {
          // 结束标记，可选处理
        } else if (eventType === 'error') {
          try { const err = JSON.parse(dataStr); statusMsg.value = `错误：${err.error || JSON.stringify(err)}` } catch { statusMsg.value = `错误：${dataStr}` }
        }
      }
    }
...
.ai-panel {
  ...
  /* avatar edge offset (negative moves toward edge) */
  --avatar-edge-offset: -8px
}
...
.avatar {
  ...
  background: color-mix(in oklab, var(--surface) 70%, #0000001a);
  border: 1px solid var(--surface-border)
}

/* 让头像更靠近面板边缘（左侧消息靠左，用户消息靠右）。使用变量便于调整。 */
.avatar {
  margin-left: var(--avatar-edge-offset)
}

.msg.user .avatar {
  margin-left: 0;
  margin-right: var(--avatar-edge-offset)
}
...
.bubble.ai,
.bubble.system {
  background: color-mix(in oklab, var(--surface) 85%, #00000010);
  border: 1px solid var(--surface-border);
  color: var(--fg)
}

.bubble.user {
  background: color-mix(in oklab, var(--brand) 86%, #ffffff);
  border: 1px solid color-mix(in oklab, var(--brand) 60%, var(--surface-border));
  color: #0b1220
}
...
.hint {
  font-size: 12px;
  color: color-mix(in oklab, var(--fg) 60%, #94a3b8);
  margin-right: auto;
  align-self: center
}
...
.close-btn {
  ...
  border: 1px solid var(--surface-border);
  background: transparent;
  color: color-mix(in oklab, var(--fg) 80%, #94a3b8);
  cursor: pointer;
...
.close-btn:hover {
  background: color-mix(in oklab, var(--surface) 88%, #00000018);
  border-color: color-mix(in oklab, var(--surface-border) 80%, #00000033)
}
