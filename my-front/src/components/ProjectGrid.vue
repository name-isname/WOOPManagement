  ...
  <section>
    <div class="mb-4 flex justify-between items-center">
      ...
      <div class="flex gap-2">
        <button class="bg-green-600 text-white px-3 py-1 rounded disabled:opacity-50 disabled:cursor-not-allowed"
          @click="showModal = true" :disabled="!backendOnline">创建新项目</button>
        <button class="bg-indigo-600 text-white px-3 py-1 rounded" @click="startAINew">新建对话</button>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      ...
      <!-- 卡片 -->
      <template v-else>
        <div v-for="(item, i) in items" :key="item.id" class="dnd-item" draggable="true" @dragstart="onDragStart(i)"
          @dragover.prevent @drop="onDrop(i)">
          <ProjectCard :item="item" :index="i + 1" @edit="onEdit" @delete="onDelete" @context="onCardContext" />
        </div>
      </template>
    </div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50">
      ...
      <div class="edit-panel" role="dialog" aria-modal="true">
        ...
        <div class="edit-footer">
          ...
          <button class="btn primary" @click="createProject" :disabled="creating">{{ creating ? '创建中…' : '创建'
          }}</button>
        </div>
    ...
    <div v-if="confirmDeleteModal" class="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div class="modal-card-card w-96" role="dialog" aria-modal="true" aria-labelledby="del-title">
        ...
        <div class="modal-actions">
          ...
          <button class="btn danger" @click="doDelete" :disabled="deleteLoading">{{ deleteLoading ? '删除中...' : '确认删除'
          }}</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="menu.visible" :style="menuStyle" class="context-menu shadow p-2 rounded">
      ...
      <button class="block px-3 py-1 hover:bg-slate-100 w-full text-left" @click="menuLinkAI">新建对话</button>
    </div>
  </section>
</template>
...
const emit = defineEmits(['start-ai', 'backend-online'])
...
function menuLinkAI() { emit('start-ai'); hideMenu() }
...
onBeforeUnmount(() => { window.removeEventListener('click', hideMenu) })
...
.icon-wrap {
  ...
  background: color-mix(in oklab, var(--brand) 18%, #fee2e2);
  color: color-mix(in oklab, var(--brand) 60%, #ef4444);
  font-size: 20px;
  border: 1px solid color-mix(in oklab, var(--brand) 8%, #00000008)
}
...
.modal-actions {
  ...
  gap: 10px;
  padding-top: 8px;
  border-top: 1px dashed color-mix(in oklab, var(--surface-border) 60%, #00000008);
  margin-top: 8px
}
...
.dnd-item:where([draggable="true"]) {
  cursor: grab
}

.dnd-item:where([draggable="true"]:active) {
  cursor: grabbing
}
...
.icon-btn:hover {
  background: color-mix(in oklab, var(--surface) 70%, #00000010)
}
...
.input:focus {
  outline: none;
  border-color: color-mix(in oklab, var(--brand) 60%, var(--surface-border));
